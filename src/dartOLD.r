...
if (sum(ind.cov[,id.col] == names(sdata))== nind ) cat ("Ids of covariate file are matching!\n") else {cat("Ids are not matching!!!!\n");stop()}
 ...
 lon.col = match( "lon", names(ind.cov))
  ...
  if (!is.na(lat.col) & !is.na(lon.col))
    {
    gout@other$latlong <- ind.cov[,c(lat.col, lon.col)]
...
# known.col <- ifelse(is.na(known.col), , known.col)
 ...
 if (length(other.col>0) )
    {
    gout@other$covariates<-ind.cov[,other.col]
