...
allVars<-dplyr::select(allVars, Label, Distance.to.Angle.Crossing..m., SiteName,Delta13C,Delta15N,CNRatio,Li:Pb)
...
#Create chemotype distance matrix
df<-allVars
df<-dplyr::select(df,Delta13C,Delta15N,CNRatio,Li:Pb)
...
df<-allVars
df<-dplyr::select(allVars, Distance.to.Angle.Crossing..m.) #should be sibs corrected nest dist?
geoDist<-df
