...
```
Of the 21076 alleles, the number of unique Maccullochella snps analysed in the DaRT sequences was `r length(uniqueSNPS)-2`. This provide high resolution of the genotypes of the larvae. The base pairs that had mutated we as follows:
...
The polymorphisms examined included SNPs and small indels in restriction enzyme recognition sites, and while none were observed larger insertions/deletions in restriction fragments could also have been utilised. The polymorphism selected for genotyping were all snps. 
...
```{r}
polymorphismPositionHistogram<-hist(as.numeric(DMac14.1567snps$X..4),main= "Histogram of SNP Position in 69 Base Pair Fragment", xlab="SNP Position on Fragment - BIN", breaks = 13, col = "lightblue", border = "pink") #a plot of the relative position of the polymorphism along the 69 base pair fragment of DNA.
...
plot(color_labels(MacHC, k = 5), main="All Maccullochella Larvae") 
...
#Plot it
plot(MChc,main=expression(paste("Genetic Relationships - ", italic("Maccullochella peelii"), " larvae 2013 cohort ")))
```
...
plot(dend1, main=expression(paste("Genetic Relationships - ", italic("Maccullochella peelii"), " larvae 2013 cohort ")))
```
