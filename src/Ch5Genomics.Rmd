...
uniqueSNPS<-unique(df$X..1)
polymorphismPositionHistogram<-hist(as.numeric(df$X..4)) #a plot of the relative position of the polymorphism along the 69 base pair fragment of DNA.
lengthDNA<-median(as.numeric(nchar(as.character(df$X..2))))
rm(df)
```
...
```{r}
#started to regex etc to pulls a,c,t,g for counts but stopped. Code here just in case I come back to it. I did it in excel so table above may do.
#dfBPchange<-df$X..3
#dfBPchange<-dfBPchange[-c(1:5)]
#head(dfBPchange)
#dfBPchange<-as.data.frame(dfBPchange)
#gsub(".*:","",dfBPchange$dfBPchange) #remove all position numbers before th3e ':'
```
...
```
As it turns out there are multiple *Maccullochella* species. All the larvae collected fall into one of four distinct clades. The above dendrograms shows all larvae (mostly Murray cod) and two known Trout cod controls, and what appears to be F1 and F2 hybrids between the two species. There were 4 hybrid and no pure trout cod larvae detected in the 92 larvae caught and sequenced from the river.
...
So there is some small but significant correlation between genetic distance and geographic distance in the Murray cod sampled. Isolation by distance does occur, even over this very small spatial scale.
...
* IBD quite marked compared to some marine species. See salmon over thousands of kilometres (same r squared) [ref: powerpoint slide]
