...
```{r, "ExtractAndDisplayClades", echo=TRUE}
#dend1 <- color_branches(MChc, k = 12)
dend1 <- color_labels(MChc, k = 12)
plot(dend1)
# Get the package:
...
A2Rplot(TChc, k =3, boxes = FALSE, col.up = "gray50")
```
...
This document maps out the physical distances with genetic distance of larvae from the Murrumbidgee collected in 2013 from 6 sites.

```{r "Load_More_Libraries", echo=TRUE}
library(Hmisc)
library(ade4)
```

This file is to add some calculations to qslLarvaeAgePlus. In particular to add:
...
```{r "Additional_Calculations_and_Ordering", echo=TRUE}
#This file is to add some calculations to qslLarvaeAgePlus. In particular to add:
#  
  #Age from Otolith Length: 74.308*[MeanOtolithLength]-4.44361
  #Hatch DoY :  [Day of Year Caught]-[Age From Otolith Length]
...
larv$nestdist<-larv$Distance.to.Angle.Crossing..m.-(1000*(larv$Day.of.Year-(larv$hatchdoy+7)))  
...
-----------------------
```{r "Liner_Regression_Model_for_Genetics_and_Distance", echo=TRUE}
...
```
This suggests that all haplogroups except 5 exist below some barrier but that only haplogroups 7,4 and 2 also exist above this barrier. On possible explanation that might be inferred from this is that adults migrating upstream for spawning are prevented from doing so by a barrier at 25000m mark but that larvae produced above the barrier are able to disperse and so are represented downstream. The big gap in apparent nests from 6000m to 14000m corresponds with the Tharwa sand slug - a long stretch of sand that has previously been the subject of remedial work because it has been believed to be a barrier to cod migration (ref).   
