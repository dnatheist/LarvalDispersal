...
ptm <- proc.time()
for (ldvRange in seq(-2000,5000, by=10)){# Possible dispersal from 2km upstream to 5 km downstream, 10m increments 
        
        larv$tmpNestDist<-larv$Distance.to.Angle.Crossing..m.-(ldvRange*(larv$Day.of.Year-(larv$hatchedDoY+7)))  
        ...
        ldvRangeVector<-append(ldvRangeVector,replicate(nrow(geodist)*nrow(geodist),ldvRange))
